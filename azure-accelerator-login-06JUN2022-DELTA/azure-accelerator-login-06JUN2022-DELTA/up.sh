...
sudo docker-compose -f /opt/docker-compose/docker-compose.yml down -v > /dev/null 2>&1
sleep 5
...
vault kv put secret/mysql url=jdbc:mysql://${ip}:3306/azx?useSSL=false password=${password} username=root > /dev/null 2>&1
